...
class ConsultationFetcher(IBaseFetcher):
    async def fetch(self, encounter_id: str) -> ConsultationProtocolV7:
        ...
        if mkb_cd is None:
            raise ValueError(f"Could not resolve MKB code for {raw_encounter['main_diagnosis_id']}")
        ...
        return ConsultationProtocolV7(
            ...
            diagnoses=[
                DiagnosisModel(code=mkb_cd, text=raw_encounter["diagnosis_text"], kind="main")
            ],
